...
export default function CartDrawer() {
  ...
  const handleCheckout = (e) => {
    ...
    const paymentText = paymentMethod === 'online' ? 'Online Payment (UPI / GPay / PhonePe / Card)' : 'Cash / Pay at Counter';
    ...
    const waText = encodeURIComponent(
      ...
      `💳 *PAYMENT METHOD:* ${paymentMethod === 'online' ? '📲 ' + paymentText : '💵 ' + paymentText}\n\n` +
      `👤 *CUSTOMER DETAILS:*\n` +
            ...
            {cartItems.length === 0 ? (
              <div className="text-center py-16 space-y-4">
                ...
                <p className="text-xs text-espresso-800/70 max-w-xs mx-auto">
                  Browse our gourmet menu and add your favorite dishes, coffee & desserts!
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                <form onSubmit={handleCheckout} className="pt-4 border-t border-cream-200 space-y-4">
                  <span className="text-xs uppercase tracking-wider font-bold text-gold-dark block">
                    Order Options & Checkout
                  </span>
                  ...
                  <div className="grid grid-cols-2 gap-2 p-1.5 rounded-2xl bg-cream-100">
                    ...
                    >
                      <CreditCard className="w-3.5 h-3.5" /> Online (UPI/Card)
                    </button>
                    <button
                      type="button"
                  ...
                  >
                    ...
                    <span>Confirm Order via WhatsApp (Grateful Message)</span>
                  </button>
                </form>
              </div>
            )}
